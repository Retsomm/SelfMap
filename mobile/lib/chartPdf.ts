...
export function generateAiPrompt(chart: PendingChart): string {
  ...
  const SIGNATURE_MAP: Record<string, { positive: string; negative: string }> = {
  ...
  const AUTHORITY_TIP: Record<string, string> = {
  ...
  const CENTER_NAME: Record<string, string> = {
    ...
    spleen: '脾中心', sacral: '薦骨中心',
    solarPlexus: '情緒中心', solar: '情緒中心',
    root: '根部中心',
  }
  const ALL_CENTER_IDS = ['head', 'ajna', 'throat', 'g', 'ego', 'spleen', 'sacral', 'solarPlexus', 'root']

  const definedCenterNames = chart.centers.map(id => CENTER_NAME[id] ?? id)
  const definedSet = new Set(chart.centers)
...
export async function downloadTransitPdf(result: CreateTransitResult): Promise<void> {
  const pendingChart: PendingChart = {
    ...
    profile: '',
    definition: '',
    centers: result.personalDefinedCenterIds,
    channels: result.personalDefinedChannelIds,
    gates: result.personalGates,
