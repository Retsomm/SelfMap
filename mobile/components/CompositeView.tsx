...
export default function CompositeView() {
  ...
  const handleDownload = async () => {
    if (!result) return
    setPdfLoading(true)
    try { await downloadCompositePdf(result, mode) }
              ...
              return (
                <View key={type} style={[s.connGroup, { borderColor: cfg.color + '55' }]}>
                  <View style={[s.connGroupHeader, { backgroundColor: cfg.bg }]}>
                    <Text style={[s.connLabel, { color: cfg.color }]}>{CONN_LABEL[type]}（{items.length}）</Text>
                    <Text style={s.connDesc}>{CONN_DESC[type]}</Text>
                  </View>
                  {items.length === 0 ? (
          ...
          <View style={s.card}>
            ...
            {(result.profileResonance?.length ?? 0) === 0 ? (
              ...
              (result.profileResonance ?? []).map(line => {
                ...
                if (!info) return null
                return (
                  <View key={line} style={s.resonanceRow}>
                    <Text style={s.resonanceLabel}>{info.title}</Text>
                    <Text style={[s.muted, { lineHeight: 20 }]}>{info.desc}</Text>
                  </View>
                )
              })
            )}
