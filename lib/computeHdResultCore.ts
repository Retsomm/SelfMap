/**
 * 共用的行星經度計算流程，供 client（computeHdResult.ts, browser WASM）與
 * server（computeHdResultServer.ts, Node WASM）兩份 Swiss Ephemeris 初始化方式共用，
 * 避免星體常數與計算步驟各自維護、精度修正只改到一邊。
 */
import { Planet, LunarPoint, CalculationFlag } from '@swisseph/core'
import {
...
const DEFAULT_FLAGS = CalculationFlag.SwissEphemeris | CalculationFlag.Speed

/** client（@swisseph/browser）與 server（swissEphServer.ts）兩份 swe instance 的最小共同介面 */
export interface HdSweAdapter {
  dateToJulianDay(date: Date): number
  calculatePosition(jd: number, body: number, flags?: number): { longitude: number }
}

export const buildHdResult = async (
  swe: HdSweAdapter,
  date: string,
  time: string,
  timezone: string,
  ...
  const designJd = getDesignJd(swe, jd, DEFAULT_FLAGS)
  const designUtc = new Date((designJd - 2440587.5) * 86400 * 1000)

  const lon = (body: number, jdVal: number) =>
    swe.calculatePosition(jdVal, body, DEFAULT_FLAGS).longitude
